"""Tests for custom WHERE conditions and JOINs functionality."""
...
def test_custom_joins(test_tables):
    """Test custom JOINs functionality."""
    config = {
        "sqlalchemy_url": DB_SQLALCHEMY_URL,
        "custom_joins": {
            "public-test_orders": {
                "type": "INNER JOIN",
                "table": "test_organizations",
                "on": "test_orders.\"organizationId\" = test_organizations.id"
            }
        },
        "custom_where_conditions": {
            "public-test_orders": "test_organizations.active = true"
        }
    }
    ...
    # Verify that JOIN and WHERE conditions were applied
    orders_stream = f"{DB_SCHEMA_NAME}-test_orders"
    
    # Should only have orders from active organizations (org_123 and org_456)
    if orders_stream in test_runner.records:
        for record in test_runner.records[orders_stream]:
            assert record["organizationId"] in ["org_123", "org_456"], \
                f"Found order with organizationId {record['organizationId']}, expected only active orgs"


def test_multiple_joins(test_tables):
    """Test multiple JOINs on a single stream."""
    # First, create a user_roles table for testing multiple joins
    engine = sa.create_engine(DB_SQLALCHEMY_URL, future=True)
    metadata_obj = sa.MetaData()
    ...
    with engine.begin() as conn:
        user_roles_table.drop(conn, checkfirst=True)
        user_roles_table.create(conn)
        
        # Insert test data
        roles = []
        for i in range(1, 16):  # For users 1-15
            roles.append({
                "user_id": i,
                "role": "admin" if i % 3 == 0 else "user"
            })
        conn.execute(user_roles_table.insert().values(roles))
    ...
    try:
                ...
                "public-test_users": [
                    {
                        "type": "LEFT JOIN",
                        "table": "test_organizations",
                        "on": "test_users.\"organizationId\" = test_organizations.id"
                    },
                    {
                        "type": "LEFT JOIN",
                        "table": "test_user_roles",
                        "on": "test_users.id = test_user_roles.user_id"
                    }
                ]
            },
            "custom_where_conditions": {
                "public-test_users": "test_organizations.active = true AND test_user_roles.role = 'admin'"
            }
        }
        ...
        # Select only test_users
        for stream in tap_catalog.streams:
            stream_name = stream.stream.split("-")[-1] if stream.stream else ""
            if stream_name == "test_users":
                for metadata in stream.metadata.values():
                    metadata.selected = True
                    if isinstance(metadata, StreamMetadata):
                        metadata.forced_replication_method = "FULL_TABLE"
        ...
        # Verify that multiple JOINs and WHERE conditions were applied
        users_stream = f"{DB_SCHEMA_NAME}-test_users"
        
        # Should only have admin users from active organizations
        if users_stream in test_runner.records:
            for record in test_runner.records[users_stream]:
                assert record["organizationId"] in ["org_123", "org_456"], \
                    f"Found user with organizationId {record['organizationId']}, expected only active orgs"
                # Can't directly verify role from record, but the filter should ensure only admins
    
    finally:
        # Clean up the user_roles table
        with engine.begin() as conn:
            conn.execute(sa.text("DROP TABLE IF EXISTS test_user_roles"))
